...
--
-- 前提: アプリ側は lib/supabase/admin.ts の createAdminClient()（SUPABASE_SECRET_KEY）
--       を使って書き込みを行うこと。secret キーは service_role ロールにマップされ RLS をバイパスする。
--       publishable (anon) からの書き込みはポリシー不在により自動的に拒否される。
